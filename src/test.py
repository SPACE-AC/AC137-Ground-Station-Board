...
class Gis:
    ...
    def getArcRadians(self) -> float:
        ...
        a += (_math.cos(self.lat1) * _math.cos(self.lat2)
              * ((_math.sin(self.dlon / 2)) ** 2))
        y = _math.sqrt(a)
        x = _math.sqrt(1 - a)
        c = 2 * _math.atan2(y, x)
        return c
...
point1 = Coordinate(13.723193, 100.515634, 0)
point2 = Coordinate(13.723297, 100.515592, 10)

gnss = Gis(point1, point2, gx=0, gy=0, gz=0)
print(f'Heading: {gnss.getHeading()}')
print(f'Elevation: {gnss.getElevationApprox()}')
print(f'Line of Sight: {gnss.getLineOfSight()} m')
